// src/app/components/BudgetChart.jsx
...
export default function BudgetChart({ transactions = [], budgets = [] }) {
  ...
  const availableMonths = budgets && budgets.length > 0
    ? [...new Set(budgets.map(budget => budget.month))].sort()
    : [];

  // Filter transactions for the selected month
  const monthTransactions = transactions && transactions.length > 0
  ...
  const expensesByCategory = monthTransactions.length > 0
  ...
  const monthBudgets = budgets && budgets.length > 0
